...
| BL-011 | Set strict-launch env vars (`VITE_SITE_URL`, `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `VITE_WAITLIST_ENDPOINT`) | Partially complete | You | Hosting vars are present in Vercel, and malformed `VITE_SITE_URL` (`https://www.namaskarai.in\\n`) was corrected in `production` + `preview` on Feb 25. Local `npm run check:launch:strict` still fails when vars are not exported in the shell/CI context. | Ensure strict-check env vars are exported in local/CI run context, then rerun `npm run check:launch:strict` |
